...
QUY ƯỚC THAM CHIẾU CỘT:
  - Chữ cái cột Excel (str)  -> 'A', 'B', 'L', 'AA'... đúng như cột trên Excel.
  - Số nguyên (int)          -> vị trí cột theo thứ tự, đếm từ 1 (cũng chấp nhận).
  - Chuỗi khác (str)         -> khớp theo TÊN tiêu đề cột (khớp gần đúng "chứa").
...
TMS = {
    "header_row": 2,
    "mst_col": "D",          # cột D  - Mã số thuế
    "status_col": "AS",      # cột AS - Trạng thái ĐKT tổ chức
    "close_date_col": "BB",  # cột BB - Ngày đóng trạng thái tổ chức
}
    ...
    "sheets": [
        {
            ...
            "header_row": 1,
            "mst_col": "B",
            "doc_col": "F",  # cột F - "Văn bản"
        },
        {
            "name": "Công văn 4532",
            "header_row": 3,
            "mst_col": "B",
            "doc_col": "Công văn 4532",  # nhãn cố định (không phải cột)
        },
        {
            "name": "524 DN",
            "header_row": 4,
            "mst_col": "A",
            "doc_col": "Công văn 1798/TCT-TTKT ngày 16/5/2023",  # nhãn cố định
        },
    ],
}
...
EINVOICE = {
    "header_row": 4,
    "cols": {
        "mst": "G",          # cột G - MST người bán/người xuất hàng
        "invoice_no": "D",   # cột D - Số hóa đơn
        "invoice_date": "E", # cột E - Ngày lập
        "status": "S",       # cột S - Trạng thái hóa đơn
        "total_tax": "M",    # cột M - Tổng tiền thuế
    },
}
